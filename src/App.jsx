...
function localLookup(text, hint) {
  ...
  // If hint exists but no local match, use hint to infer
  if (hint) {
    const h = hint.trim();
    // Check if hint looks like a known author/source
    const knownAuthors = LOCAL_DB.filter(e => e.s.toLowerCase().includes(h.toLowerCase()));
    if (knownAuthors.length > 0) {
      const cat = knownAuthors[0].c;
      return { source: h, category: cat, confidence: "medium", local: true };
    }
    // Hint exists but unknown — still useful, skip API with partial info
    return null;
  }
  return null;
}
...
export default function Keeper() {
  ...
  const [sortBy, setSortBy] = useState("default"); // default | confidence | alpha | category
...
Unknown if unsure. Be concise with sources. Return one object per input.`,
  ...
  const processEntries = async (inputText, appendMode = false) => {
    ...
    // Dedupe within batch + against existing quotes
    ...
    const newQuotes = unique.map((p, i) => {
      ...
      // No result (API failed or missed)
      return {
        id: (Date.now() + i).toString(), text: p.text,
    ...
    setStats({
      ...
      api: apiResults.size,
      failed: apiFailed ? needsApi.length - apiResults.size : 0,
      total: unique.length,
      dupes
    });
    setDupeCount(dupes);
    ...
    <div style={Z.wrap}><style>{baseCSS}</style>
      <div style={Z.landing}>
        ...
        <div style={Z.inputCard}>
          <textarea ref={inputRef} style={Z.bigTextarea} value={rawInput} onChange={e => setRawInput(e.target.value)}
          ...
          <div style={Z.inputFooter}>
            ...
            <button style={{ ...Z.processBtn, opacity: !rawInput.trim() ? 0.4 : 1 }} onClick={handleProcess} disabled={!rawInput.trim()}>Organize my collection →</button>
          </div>
        </div>
  ...
  return (
    <div style={Z.wrap}><style>{baseCSS}</style>
      ...
      {filtered.length === 0 && <div style={Z.empty}><p style={{ fontSize: 14, color: "#9B9A97" }}>Nothing matches your filters.</p></div>}
    </div>
  );
}
