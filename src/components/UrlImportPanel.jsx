...
export default function UrlImportPanel({ onLoad }) {
  ...
  const fetchUrl = async (trimmed, mode, { showLoading = false, openModal = false } = {}) => {
    ...
    try {
      ...
      if (openModal && (!data.lines || data.lines.length === 0))
        throw new Error("No text content found on that page");
  ...
  return (
    <div style={{ padding: "20px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 4,
            border: "none",
            background: loading ? "var(--cp-border)" : "#2383E2",
            color: "#fff",
            fontSize: 13,
            fontWeight: 600,
            fontFamily: "inherit",
        ...
        {EXTRACT_MODES.map((m) => (
          <button
            ...
            style={{
              ...
              borderRadius: 50,
              border:
                extractMode === m.value ? `1px solid ${CP_ACCENT}` : "1px solid var(--cp-border)",
      ...
      <p style={{ marginTop: 12, fontSize: 11, color: "var(--cp-text-faint)" }}>
        Fetches the page and extracts text content. Choose an extraction mode to filter what gets
        pulled.
      </p>
    </div>
  );
