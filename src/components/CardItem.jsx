    ...
    >
      ...
      {(isSwipingLeft || isSwipingRight) && (
        <div
          style={{
            ...
            inset: 0,
            borderRadius: 6,
            background: isSwipingLeft ? "var(--cp-error-bg)" : "var(--cp-warning-bg)",
            display: "flex",
            alignItems: "center",
      ...
      <div
        ...
        style={{
          ...
          ...{
            boxShadow:
              [
                stripeColor ? `inset 3px 0 0 ${stripeColor}` : null,
                isOverTarget ? `inset 0 2px 0 ${CP_ACCENT}` : null,
              ]
                .filter(Boolean)
                .join(", ") || undefined,
          },
        ...
        onMouseEnter={(e) => {
          const a = e.currentTarget.querySelector(".ca");
      ...
      >
        <div style={cardStyles.top}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ position: "relative" }}>
              <span
                className={clsx("inline-cat", {
                  "save-pulse": isSavedPulse && savedPulseField === "category",
                })}
                style={{
                  ...styles.tag,
                  background: col.bg,
                  ...
                  alignItems: "center",
                  gap: 2,
                  ...(isMobile ? { padding: "4px 10px", fontSize: 12 } : {}),
                }}
                onClick={(e) => {
                  e.stopPropagation();
        ...
        {isEd && !isMobile ? (
          ...
          <>
            <p
              style={{ ...cardStyles.txt, cursor: "text" }}
              onClick={() => {
                if (!isEd) startEditing(q.id);
              }}
            >
              {(() => {
                const full = displayText(q);
                const truncatable = full.length > QUOTE_TRUNCATE_CHARS;
                const shown =
            ...
            <div style={cardStyles.srcRow}>
              ...
              {isInlineEditing && inlineEditField === "source" ? (
                ...
                <>
                  <span
                    className={clsx("inline-src", {
                      "save-pulse": isSavedPulse && savedPulseField === "source",
                    })}
                    style={cardStyles.src}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!isEd) startInlineEdit(q.id, "source");
                    }}
                  >
