        ...
        <Menu.Portal>
          <Menu.Positioner side="bottom" align="end" sideOffset={4} style={{ zIndex: 100 }}>
            <Menu.Popup style={menuContentStyle} onClick={(e) => e.stopPropagation()}>
              ...
              <Menu.Item
                className={clsx({
                  "overflow-menu-item-fav": q.favorite,
                  "overflow-menu-item": !q.favorite,
                })}
                style={styles.overflowMenuItem}
                onClick={() => {
                  actionProps.onFav(q.id);
                }}
              >
                <Star
                  size={14}
...
const CONF_STRIPE = {
  high: "var(--cp-conf-high)",
  medium: "var(--cp-conf-medium)",
  low: "var(--cp-conf-low)",
};
...
const TableRow = memo(function TableRow({
  ...
  const sortableStyle = isDragging ? { transform: CSS.Transform.toString(transform) } : undefined;

  const [expanded, setExpanded] = useState(false);

  const longPress = useLongPress(() => toggleSel(q.id), 400);
    ...
    switch (colKey) {
      case "content": {
        ...
        return (
          <div
            key="content"
            style={COL_BASE.content}
            onClick={() => {
              if (!isEd) setEditingId(q.id);
            }}
          >
            {isEd ? (
              <EditForm
              ...
              <p
                title={full}
                style={{
                  ...styles.entryTextCompact,
                  overflow: "hidden",
                  textOverflow: "ellipsis",
                  whiteSpace: "nowrap",
                }}
              >
                <HighlightText text={full} term={searchTerm} />
              </p>
            ) : (
      ...
      case "source":
        return (
          <div
            key="source"
            className={clsx("src-col", {
              "save-pulse": isSavedPulse && savedPulseField === "source",
            })}
            style={COL_BASE.source}
          >
            {isInlineEditing && inlineEditField === "source" ? (
              ...
              <>
                <span
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!isEd) setInlineEdit({ id: q.id, field: "source" });
                  }}
                >
                  <HighlightText text={q.source} term={searchTerm} />
                </span>
                <Pencil
                  className="edit-hint"
                  size={11}
      ...
      case "category":
        return (
          <div
            key="category"
            className={clsx({ "save-pulse": isSavedPulse && savedPulseField === "category" })}
            style={{ ...COL_BASE.category, gap: 6, overflow: "visible", position: "relative" }}
          >
            <span
              ...
              style={{
                ...
                overflow: "hidden",
                textOverflow: "ellipsis",
                maxWidth: "min(110px, 25vw)",
                cursor: "pointer",
              }}
              onClick={(e) => {
                e.stopPropagation();
                if (!isEd) setInlineEdit({ id: q.id, field: "category" });
              }}
              title={q.category}
            >
              {q.category}
              <ChevronDown className="edit-hint" size={10} strokeWidth={2} color="currentColor" />
            </span>
...
const TableView = memo(function TableView({ filtered, toolbarHeight = 44, dndReorderRef }) {
  ...
  const listFingerprint = filtered
    .slice(0, 8)
    .map((q) => q.id)
    .join(",");
      ...
      <div style={isMobile ? { minWidth: 560 } : undefined}>
        {filtered.length > 0 && (
          <div
            style={{
              ...styles.tHead,
              ...(isMobile ? { position: "static" } : { top: toolbarHeight }),
            }}
          >
            ...
            <div
              ...
              data-tip="Select all"
              style={{ ...styles.chkW, ...(allSelected ? { opacity: 1 } : {}) }}
            >
              <div
                className="checkbox-visual"
