        ...
        >
          <div style={{ ...styles.shareBanner, margin: 0 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <Eye size={15} strokeWidth={1.5} /> You're viewing a shared collection ({quotesLength}{" "}
              entries)
            </span>
            <button
      ...
      {apiError && (
        ...
        >
          <div style={{ ...styles.errorBar, margin: 0 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
              <TriangleAlert size={14} strokeWidth={2} /> {apiError}
            </span>
            <div style={{ display: "flex", gap: 8 }}>
              {failedEntries.length > 0 && (
                <button style={styles.retryBtn} onClick={retryFailed}>
        ...
        >
          <div style={{ ...styles.statsBar, margin: 0 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
              <Zap size={13} strokeWidth={2} /> <strong>{stats.local}</strong> matched locally
            </span>
            {stats.lookup > 0 && (
              <>
                <span style={styles.statDot} />
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                  <Globe size={13} strokeWidth={2} /> <strong>{stats.lookup}</strong> found online
                </span>
              </>
            )}
            <span style={styles.statDot} />
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
              <Bot size={13} strokeWidth={2} /> <strong>{stats.api}</strong> identified by AI
            </span>
            {stats.failed > 0 && (
              <>
                <span style={styles.statDot} />
                <span
                  style={{ color: CLR_RED, display: "inline-flex", alignItems: "center", gap: 4 }}
                >
                  <CircleX size={13} strokeWidth={2} /> <strong>{stats.failed}</strong> failed
                </span>
              </>
            )}
            {stats.dupes > 0 && (
              <>
                <span style={styles.statDot} />
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                  <RefreshCw size={13} strokeWidth={2} /> <strong>{stats.dupes}</strong> duplicate
                  {stats.dupes > 1 ? "s" : ""} skipped
                </span>
              </>
            )}
            <button
              style={styles.statsDismiss}
              aria-label="Dismiss processing summary"
      ...
      {showReview && (
        ...
        >
          <div style={{ ...styles.attentionBar, margin: 0 }}>
            ...
            <button
              style={{ ...styles.attentionBtn, background: "#92400E" }}
              onClick={() => {
                setReviewQueue([]);
                setEditingId(null);
              }}
        ...
        >
          <div style={{ ...styles.attentionBar, margin: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span>
                {unknownCount === 1 ? "entry needs" : "entries need"} your attention — source or
                category is missing
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <button
                ...
                data-tip="Step through entries that need attention"
              ...
              <button
                className="ui-tip ui-tip-left attention-dismiss"
                data-tip="Dismiss"
                aria-label="Dismiss review reminder"
                style={styles.attentionDismiss}
                onClick={() => setDismissedAtCount(unknownCount)}
              >
                &times;
              </button>
            </div>
