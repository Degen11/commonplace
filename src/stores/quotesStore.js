...
function loadDeletedIds() {
  const now = Date.now();
  return loadFromStorage(LS_DELETED_IDS, Array.isArray).filter(
    (e) => e && typeof e.deletedAt === "number" && now - e.deletedAt < TOMBSTONE_TTL_MS,
  );
}
...
export const useQuotesStore = create(
  subscribeWithSelector((set, get) => ({
    ...
    quotes: isShareHash()
      ? []
      : loadFromStorage(LS_QUOTES, (a) => Array.isArray(a) && a.length > 0).filter(
    ...
    columnOrder: loadFromStorage(
      ...
      (p) =>
        Array.isArray(p) &&
        p.length === REORDERABLE_COLS.length &&
        REORDERABLE_COLS.every((c) => p.includes(c)),
      () => [...REORDERABLE_COLS],
    ),
    ...
    handleCloudData: (cloudQuotes, cloudCats, cloudCollections) => {
      ...
      const filteredCloudQuotes =
        tombstoneIds.size > 0 && cloudQuotes?.length > 0
    ...
    cleanCollectionRefs: (deletedQuoteIds) => {
      ...
      set((state) => {
        ...
        const next = state.collections.map((c) => {
          ...
          if (filtered.length !== c.quoteIds.length) {
            changed = true;
            return { ...c, quoteIds: filtered };
          }
          return c;
        });
...
useQuotesStore.subscribe(
  ...
  (collections) => {
    saveToStorage(LS_COLLECTIONS, collections);
  },
);
      ...
      if (e.key === LS_QUOTES && e.newValue) {
        ...
        if (Array.isArray(parsed)) {
          useQuotesStore.setState((state) => {
            ...
            return incoming.length > 0 ? { customCats: [...state.customCats, ...incoming] } : {};
          });
        }
